...
void horizon_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
        // cout << "horizon111" << endl;
        double t1 = omp_get_wtime();
        vector<pcl::PointCloud<PointType>> pl_buff(N_SCANS); //横向扫，6条线,这种每个点带有line号，也确实是一条水平的线
        vector<vector<orgtype>> typess(N_SCANS);
        pcl::PointCloud<PointType> pl_full, pl_corn, pl_surf;

        uint plsize = msg->point_num;
        // ROS_INFO("lalala......", plsize);
        ...
        for (uint i = 1; i < plsize; i++)
        {
                ...
                if ( ( msg->points[ i ].line < N_SCANS ) && ( !IS_VALID( msg->points[ i ].x ) ) && ( !IS_VALID( msg->points[ i ].y ) ) && ( !IS_VALID( msg->points[ i ].z ) ) && msg->points[ i ].x > 0.7 )
                {
                        ...
                        if ( ( msg->points[ i ].x > 2.0 ) && ( ( ( msg->points[ i ].tag & 0x03 ) != 0x00 )  ||  ( ( msg->points[ i ].tag & 0x0C ) != 0x00 ) ))
                        {
                        ...
                        pl_full[i].curvature = msg->points[i].offset_time / float(1000000); // use curvature as time of each laser points 利用曲率作为每个激光点的时间

                        if ((std::abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) || (std::abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7) || (std::abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7))
                        {
                                pl_buff[msg->points[i].line].push_back(pl_full[i]); // pl_buff：去除坏点之后的集合
                        }
                }
        }
        ...
        for (int j = 0; j < N_SCANS; j++)
        {
                ...
                types.resize(plsize);
                plsize--;
                for (uint pt_idx = 0; pt_idx < plsize; pt_idx++) //遍历每一个点
                {
                        types[pt_idx].range = pl[pt_idx].x * pl[pt_idx].x + pl[pt_idx].y * pl[pt_idx].y; //每一个点距离雷达的距离
                        vx = pl[pt_idx].x - pl[pt_idx + 1].x;                                            //记录当前点的跳变
                ...
                // cout << "在1" << endl;
                // cout << "pl" << pl.points.size() << endl;
                give_feature(pl, types, pl_corn, pl_surf); //但是没有提取面点，只做了坏点剔除
                // cout << "pl_surf" << pl_surf.points.size() << endl;
        }
        // cout << "corn " << pl_corn.points.size() << endl;
        // cout << "surf :  " << pl_surf.points.size() << "all :  " << pl_full.points.size() << endl;
                  ...
                  pcl::PointCloud<PointType> &pl_surf)
{
        ...
        // Surf
        // cout << "qian  " << plsize << endl;
        plsize2 = (plsize > group_size) ? (plsize - group_size) : 0;
        // cout << "hou    " << plsize2 << endl;
        ...
        for (uint i = head; i < plsize2; i += g_LiDAR_sampling_point_step)
        {
                ...
                // cout << g_if_using_raw_point << "在5  " << endl;
